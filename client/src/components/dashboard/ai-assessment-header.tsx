...
const filteredProjects = React.useMemo(() => {
  ...
  if (!currentAssessment) {
    return (
      <div className="p-4 md:p-8 text-center text-gray-600 bg-gray-50 rounded-xl">
        <AlertCircle className="w-8 h-8 md:w-10 md:h-10 mx-auto mb-2 text-gray-400" />
        <p className="text-sm md:text-base">No assessment data available</p>
        <button 
          onClick={() => window.location.reload()}
          className="mt-4 px-3 py-1.5 md:px-4 md:py-2 bg-gray-100 hover:bg-gray-200 rounded-lg text-gray-700 font-medium text-sm"
        >
          Refresh
        </button>
      </div>
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-lg p-4 md:p-6 border border-blue-200 shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4 mb-4 md:mb-6">
            <div className={`rounded-lg p-3 md:p-4 border-2 ${getStatusBgColor(analysis?.overallPortfolioRagStatus)} relative`}>
              <div className="flex items-center space-x-2 md:space-x-3">
                <div className={`w-3 h-3 md:w-4 md:h-4 rounded-full ${getStatusColor(analysis?.overallPortfolioRagStatus)}`}></div>
                <div>
                  <p className="font-bold text-md md:text-lg text-gray-900 capitalize">
            ...
            <div className="bg-gradient-to-br from-yellow-50 to-amber-50 rounded-lg p-3 md:p-4 border-2 border-yellow-200">
              <div className="flex items-center space-x-2 md:space-x-3">
                <div className="w-8 h-8 md:w-10 md:h-10 bg-gradient-to-r from-yellow-500 to-amber-500 rounded-lg flex items-center justify-center shadow-sm">
                  <Star className="h-4 w-4 md:h-5 md:w-5 text-white" />
                </div>
                <div>
                  <p className="font-bold text-md md:text-lg text-gray-900">{strategicProjectsData.total}</p>
                  <p className="text-xs md:text-sm text-gray-600">Strategic Projects</p>
                  {strategicProjectsData.total > 0 && (
                    <div className="text-xs text-gray-500 mt-1">
                      {Object.entries(strategicProjectsData.statusCounts).map(([status, count]) => (
                        <span key={status} className="mr-1 md:mr-2">
                          {status}: {count}
                        </span>
                      ))}
                    </div>
            ...
            <div className={`bg-gradient-to-br ${healthScoreDisplay.color.bg} rounded-lg p-3 md:p-4 border-2 ${healthScoreDisplay.color.border}`}>
              <div className="flex items-center space-x-2 md:space-x-3">
                <div className={`w-8 h-8 md:w-10 md:h-10 bg-gradient-to-r ${healthScoreDisplay.gradient} rounded-lg flex items-center justify-center shadow-sm`}>
                  <Activity className="h-4 w-4 md:h-5 md:w-5 text-white" />
                </div>
                <div>
                  <p className={`font-bold text-md md:text-lg ${healthScoreDisplay.color.text}`}>
                    {healthScoreDisplay.value}
                  </p>
                  <p className="text-xs md:text-sm text-gray-600">Overall Health Score</p>
